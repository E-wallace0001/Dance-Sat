...
void Find_Partial_Bijection(group_s* end, group_s* variable_container,formula_atribute* problem, group_s* group1, hash_t* h_table){
...
void Find_Bijection(group_s* variable_container,formula_atribute* problem, group_s* group1, lut* table, set_s* graph){
	...
	//set_s* graph = MakeSet();
	//ExtendSet( table->first, graph);
	//ExtendSet( table->second, graph);
...
void Reset_Guesses_Till(formula_atribute* problem){
	...
	while( 1 ){
		...
		if ( problem->guessed_literals->end==NULL){
			printf(" rest guessed lite empty \n");
			exit(0);
		}
...
int main(int argc, char* argv[]){
	...
	problem-> h_table				= hasht_create( 65537);
				...
				*guessed													= variable;
				...
				while( !propagate( abs( variable ), problem)  ){
					...
					if( temp_bool == 0 ){
						...
						Find_Partial_Bijection( problem->first_removed[ abs( variable ) ] , variable_container, problem, group, problem->h_table);
					...
					while(1 ){
						...
						if(group == problem->first_removed[ variable ] && abs(*(int*)l1->data) == variable ){
							...
							problem->assigned[ abs( variable) ]			= 0;
							problem->propagated[abs( variable)] 			= 0;
							
							problem->prop_cause [ variable ] 	= NULL;
							problem->init_neg[variable] 			= NULL;
					...
					if ( temp_bool == 0 ){
						...
						Find_Bijection(variable_container,problem, group, conflict, implication);
